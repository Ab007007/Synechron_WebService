...
public class ValidateResponseTime {
	...
	@Test(priority = 0)
	public void responseInSec() {
		long time = RestAssured.given().headers("Authorization", barrierToken).headers("Content-type", "application/json").log()
				.all().when().get("/repos/AravindaHB/AppiumAllIPO").timeIn(TimeUnit.SECONDS);
		
		System.out.println("Response time in sec : " + time);
	}
